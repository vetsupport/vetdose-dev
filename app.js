...
function checkRegistration() {
  ...
  if (auth === 'ok' && user) {
    // Ya autenticado — restaurar nombre en header
    try {
...
function selectTitle(t) {
  ...
  if (h) h.value = t;
  ['Dr.','Dra.'].forEach(function(tt) {
    var btn = document.getElementById('reg-btn-' + (tt === 'Dr.' ? 'dr' : 'dra'));
    if (!btn) return;
    var active = tt === t;
    btn.style.background = active ? '#1a5c38' : '#fff';
    btn.style.color = active ? '#fff' : '#666';
    btn.style.borderColor = active ? '#1a5c38' : '#ddd';
  });
}
